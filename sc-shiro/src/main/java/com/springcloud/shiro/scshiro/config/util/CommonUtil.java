package com.springcloud.shiro.scshiro.config.util;

import com.alibaba.fastjson.JSONObject;
import com.springcloud.common.sccommon.vo.CommonVO;
import com.springcloud.shiro.scshiro.config.filter.CommonJsonException;
 ...
 * @create: 2020-08-14 11:47:33
 */
public class CommonUtil {
    /**
     * 返回一个info为空对象的成功消息的json
     */
    public static CommonVO successJson() {
     ...
     * 返回一个返回码为100的json
     */
    public static CommonVO successJson(Object info) {
        return CommonVO.success(info);
    }

    /**
     * 返回错误信息JSON
     */
    public static CommonVO errorJson() {
     ...
     */
    public static CommonVO successPage(final JSONObject requestJson, List<JSONObject> list, int totalCount) {
        int pageRow = requestJson.getIntValue("pageRow");
        int totalPage = getPageCounts(pageRow, totalCount);
        CommonVO result = successJson();
     ...
     * @param list 查询分页对象list
     */
    public static CommonVO successPage(List<JSONObject> list) {
        CommonVO result = successJson();
        JSONObject info = new JSONObject();
        info.put("list", list);
        result.setData(info);
    ...
    public static void hasAllRequired(final JSONObject jsonObject, String requiredColumns) {
        if (StringUtils.isNotEmpty(requiredColumns)) {
            ...
            if (StringUtils.isNotEmpty(missCol.toString())) {
                jsonObject.clear();
